...
def events_to_image(xs, ys, ps, sensor_size=(680, 1200), interpolation='bilinear', padding=False):
    """
...
def voxel_grids_fixed_n_torch(xs, ys, ts, ps, B, n, sensor_size=(680, 1200), temporal_bilinear=True):
    """
...
def voxel_grids_fixed_t_torch(xs, ys, ts, ps, B, t, sensor_size=(680, 1200), temporal_bilinear=True):
    """
...
def events_to_voxel_timesync_torch(xs, ys, ts, ps, B, t0, t1, device=None, np_ts=None, sensor_size=(680, 1200), temporal_bilinear=True):
    """
...
def events_to_voxel_torch(xs, ys, ts, ps, B, device=None, sensor_size=(680, 1200), temporal_bilinear=True):
    """
...
def events_to_neg_pos_voxel_torch(xs, ys, ts, ps, B, device=None,
        sensor_size=(680, 1200), temporal_bilinear=True):
    """
    Turn set of events to a voxel grid tensor, using temporal bilinear interpolation.
...
def events_to_voxel(xs, ys, ts, ps, B, sensor_size=(680, 1200), temporal_bilinear=True):
    """
    Turn set of events to a voxel grid tensor, using temporal bilinear interpolation
    Parameters
    ----------
