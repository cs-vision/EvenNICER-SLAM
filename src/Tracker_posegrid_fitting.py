        ...
        for idx, gt_color, gt_depth, gt_c2w in pbar:
            ...
            if idx == 0 or self.gt_camera:
                ...
                with torch.no_grad():
                    ...
                    pred_c2w_homo = torch.cat([pred_c2w, torch.tensor([[0,0,0,1]])], dim=0)
                ...
                # pose_enc1_grad, pose_enc2_grad = Variable(pose_enc1, requires_grad=True), Variable(pose_enc2, requires_grad=True) # maybe not necessary in new pytorch
                trans_enc1_grad, trans_enc2_grad = Variable(trans_enc1, requires_grad=True), Variable(trans_enc2, requires_grad=True) # maybe not necessary in new pytorch
                quat_enc1_grad, quat_enc2_grad = Variable(quat_enc1, requires_grad=True), Variable(quat_enc2, requires_grad=True) # maybe not necessary in new pytorch
                # prime1_grad, prime2_grad = Variable(prime1, requires_grad=True), Variable(prime2, requires_grad=True) # maybe not necessary in new pytorch
                trans_prime1_grad, trans_prime2_grad = Variable(trans_prime1, requires_grad=True), Variable(trans_prime2, requires_grad=True) # maybe not necessary in new pytorch
                quat_prime1_grad, quat_prime2_grad = Variable(quat_prime1, requires_grad=True), Variable(quat_prime2, requires_grad=True) # maybe not necessary in new pytorch
                ...
                optimizer.param_groups[1]['lr'] = cfg['PoseGrid']['trans_enc_para_list']
                optimizer.param_groups[2]['lr'] = cfg['PoseGrid']['quat_enc_para_list']
                ...
                del candidate_decoder_para, candidate_trans_enc_para_list, candidate_quat_enc_para_list, candidate_trans_prime_para_list, candidate_quat_prime_para_list
                ...
                candidate_enc1 = torch.cat([candidate_pose_enc1, candidate_prime1])
                candidate_enc2 = torch.cat([candidate_pose_enc2, candidate_prime2])
